...
export default function BulkUploadModal({ open, onClose, existingUsers = [], onSuccess }) {
  ...
  const upload = async (file) => {
    ...
    try {
      ...
      const total =
        cleaned.admins.length +
        cleaned.monitors.length +
        cleaned.faculties.length +
        cleaned.students.length;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-lg max-w-lg w-full p-6 shadow-lg">
        ...
        <p className="text-sm text-gray-600 mb-3">
          ...
          <br />
          Students: semester, batchYear, section, shift, status.
        </p>
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button onClick={onClose} className="px-4 py-2 rounded border cursor-pointer hover:bg-gray-50">
            Cancel
          </button>
          <button
            disabled={loading}
